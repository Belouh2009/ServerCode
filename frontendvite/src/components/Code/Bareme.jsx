...
const Bareme = () => {
  ...
  const fetchBaremes = () => {
    setLoading(true);
    axios
      .get("http://192.168.88.47:8087/bareme/all")
      .then((response) => {
  ...
  useEffect(() => {
    const fetchDates = async () => {
      try {
        const response = await axios.get("http://192.168.88.47:8087/bareme/dates");
      ...
      const fetchCategories = async () => {
        try {
          const response = await axios.get(
            `http://192.168.88.47:8087/bareme/categories?date=${selectedDate.value}`
          );
  ...
  useEffect(() => {
    if (selectedDate && selectedCategorie) {
      const fetchIndices = async () => {
        try {
          const response = await axios.get(
            `http://192.168.88.47:8087/bareme/indices?date=${selectedDate.value}&categorie=${selectedCategorie.value}`
          );
  ...
  const sendToBackend = (data) => {
    fetch("http://192.168.88.47:8087/bareme/import", {
      method: "POST",
      headers: {
