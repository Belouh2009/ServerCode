...
export default function ContentUsers({ status, setStatus }) {
  ...
  const validateUser = async (matricule) => {
    try {
      await axios.post(`http://localhost:8087/utilisateur/valide/${matricule}`);
  ...
  const blockUser = async (matricule) => {
    try {
      await axios.post(`http://localhost:8087/utilisateur/blocke/${matricule}`);
  ...
  const UserImage = ({ imageName }) => {
    ...
    const imgSrc = isDefaultImage
      ? userIcon
      : `http://localhost:8087/uploads/${imageName}`;
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        "http://localhost:8087/utilisateur/update-profile",
        formData,
        {
          headers: {
