...
export default function ContentUsers({ status, setStatus }) {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const response = await axios.get(
        `http://192.168.88.47:8087/utilisateur?status=${status}`
      );
  ...
  const updateNonValideCount = async () => {
    try {
      const response = await axios.get(
        "http://192.168.88.47:8087/utilisateur/non-valide/count"
      );
  ...
  const validateUser = async (matricule) => {
    try {
      await axios.post(`http://192.168.88.47:8087/utilisateur/valide/${matricule}`);
  ...
  const blockUser = async (matricule) => {
    try {
      await axios.post(`http://192.168.88.47:8087/utilisateur/blocke/${matricule}`);
  ...
  const UserImage = ({ imageName }) => {
    ...
    const imgSrc = isDefaultImage
      ? userIcon
      : `http://192.168.88.47:8087/uploads/${imageName}`;
