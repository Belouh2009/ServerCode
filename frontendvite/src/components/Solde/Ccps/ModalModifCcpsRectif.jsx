...
const ModalModifCcpsRect = ({ open, onClose, agent, onSuccess, rubriques = [] }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await axios.put(
                `http://192.168.88.53:8088/agentsCcpsRect/modifier/${agent.key}`,  // Utilise 'key' dans l'URL
                updatedAgentData,
            ...
            Swal.fire("Erreur", `Impossible de rectifier le certificat : ${msg}`, "error");
        }
    };
    ...
    return (
        <Modal
            title="Modification de Certificat de Cessation de Paiement de la Solde"
            centered
            open={open}
            onCancel={onClose}
            width={1200}
            ...
            <Row gutter={16}>
                <Col span={10} className="form-container">
                    ...
                    <Form layout="vertical">
                        <Row gutter={16}>
                            <Col span={12}>
                                <Form.Item label="ID Certificat">
                                    <Input name="idCertificatRect" value={idCertificatRect} onChange={(e) => setIdCertificatRect(e.target.value)} />
                                </Form.Item>
                            ...
                            <Col span={12}>
                                <Form.Item label="Matricule" required>
                                    <Input name="matricule" value={matricule} onChange={(e) => setMatricule(e.target.value)} />
                                </Form.Item>
                            </Col>
                        </Row>
                        ...
                        <Row gutter={16}>
                            <Col span={6}>
                                <Form.Item label="Enfant">
                                    <Input type="number" name="enfant" value={enfant} onChange={(e) => setEnfant(e.target.value)} />
                                </Form.Item>
                            </Col>
                        ...
                        <Row gutter={16}>
                            ...
                            <Col span={8}>
                                <Form.Item label="Indice">
                                    <Input name="indice" value={indice} onChange={(e) => setIndice(e.target.value)} style={{ color: "black" }} />
                                </Form.Item>
                            </Col>
                        </Row>
                        ...
                        <Row gutter={16}>
                            <Col span={12}>
                                <Form.Item label="Acte">
                                    <Input name="acte" value={acte} onChange={(e) => handleChangeMain(e.target.value, "acte")} />
                                </Form.Item>
                            </Col>

                            <Col span={12}>
                                <Form.Item label="Référence">
                                    <Input name="referenceActe" value={referenceActe} onChange={(e) => handleChangeMain(e.target.value, "referenceActe")} />
                                </Form.Item>
                            </Col>
                        </Row>

                        <Form.Item label="Date Acte">
                            <Input type="date" name="dateActe" value={dateActe ? new Date(dateActe).toISOString().split("T")[0] : ""}
                        ...
                        <Row gutter={16}>
                            <Col span={12}>
                                <Form.Item label="Date de Cessation de Service">
                                    <Input type="date" name="dateCessation" value={dateCessation ? new Date(dateCessation).toISOString().split("T")[0] : ""}
                            ...
                            <Col span={12}>
                                <Form.Item label="Date Fin de Paiement">
                                    <Input type="date" name="dateFinPai" value={dateFinPai ? new Date(dateFinPai).toISOString().split("T")[0] : ""}
                ...
                <Col span={7} className="form-container">
                    ...
                    <Form layout="vertical">
                        <Form.Item label="Montants">
                            <Input name="montant" value={montant} onChange={(e) => setMontant(e.target.value)} />
                        </Form.Item>

                        <Form.Item label="Reference">
                            <Input type="text" name="referenceRecette" value={referenceRecette} onChange={(e) => setReferenceRecette(e.target.value)} />
                        </Form.Item>

                        <Form.Item label="Date d'ordre de Recette">
                            <Input type="date" name="dateOrdreRecette" value={dateOrdreRecette ? new Date(dateOrdreRecette).toISOString().split("T")[0] : ""}
                                onChange={(e) => setDateOrdreRecette(e.target.value)} />
                        </Form.Item>

                        <Form.Item label="Date debut paiement">
                            <Input type="date" name="dateDebut" value={dateDebut ? new Date(dateDebut).toISOString().split("T")[0] : ""}
                                onChange={(e) => setDateDebut(e.target.value)} />
                        </Form.Item>

                        <Form.Item label="Date dernier paiement">
                            <Input type="date" name="dateDernierPai" value={dateDernierPai ? new Date(dateDernierPai).toISOString().split("T")[0] : ""}
