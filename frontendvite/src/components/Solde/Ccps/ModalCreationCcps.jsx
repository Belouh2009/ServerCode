...
const ModalCreation = ({ open, onClose, formData, setFormData, formFields, setFormFields, onSuccess }) => {
    ...
    const [loadingCorps, setLoadingCorps] = useState([]);
    ...
    useEffect(() => {
        ...
        axios.get("http://192.168.88.53:8088/rubriquesolde/ids")
            .then((response) => {
    ...
    const handleSubmit = async () => {
        ...
        if (!formData.nom || !formData.prenom || !formData.matricule || !formData.corps || !formData.grade || !formData.indice) {
        ...
        try {
            ...
            const lastIdResponse = await fetch("http://192.168.88.53:8088/certificatsCcps/lastId");
            ...
            const dataToSend = {
                ...formData,
                certificat: {
                    id_certificat: `${String(parseInt(lastId.split("-")[0], 10) + 1).padStart(4, "0")}-${new Date().getFullYear()}`,
                    date_creation,
                    ajout_par: username,
                    modif_par: "Aucun",
                },
            ...
            const response = await fetch("http://192.168.88.53:8088/agentsCcps/enregistre", {
                method: "POST",
    ...
    return (
        <Modal title={<div style={{ textAlign: "center" }}>Création de Certificat de Cessation de Paiement de la Solde </div>} centered open={open} onCancel={onClose} width={1100} footer={null}>
            <Row gutter={16}>
                    ...
                    <Form layout="vertical">
                        <Row gutter={16}>
                            ...
                            <Col span={12}>
                                <Form.Item label="Matricule" required>
                                    <Input name="matricule" value={formData.matricule || ""} onChange={handleChangeMain} placeholder="Entrer le matricule" />
                                </Form.Item>
                            </Col>
                        </Row>

                        <Form.Item label="Nom" required>
                            <Input name="nom" value={formData.nom || ""} onChange={handleChangeMain} placeholder="Entrer le nom" />
                        </Form.Item>
                        <Form.Item label="Prénom" required>
                            <Input name="prenom" value={formData.prenom || ""} onChange={handleChangeMain} placeholder="Entrer le prénom" />
                        </Form.Item>
                        ...
                        <Form.Item label="Cessation du Service">
                            <Select
                                ...
                                onChange={(value) => handleChangeMain(value, "cessationService")}
                            >
                                <Select.Option value="Retraité pour limite d'âge">Retraité pour limite d'âge</Select.Option>
                                <Select.Option value="Décès">Décès</Select.Option>
                            </Select>
                        </Form.Item>
                        ...
                        <Row gutter={16}>
                            <Col span={6}>
                                <Form.Item label="Enfant">
                                    <Input type="number" name="enfant" value={formData.enfant || ""} onChange={handleChangeMain} placeholder="00" />
                                </Form.Item>
                            </Col>

                            <Col span={6}>
                                <Form.Item label="Zone">
                                    <Input type="number" name="zone" value={formData.zone || ""} onChange={handleChangeMain} placeholder="00" />
                                </Form.Item>
                            </Col>
                        ...
                        <Row gutter={16}>
                            <Col span={8}>
                                <Form.Item label="Corps" required>
                                    <Select value={formData.corps || ""} onChange={handleCorpsChange} placeholder="Sélectionner le corps" showSearch>
                            ...
                            <Col span={8}>
                                <Form.Item label="Grade" required>
                                    <Select value={formData.grade || ""} onChange={handleGradeChange} placeholder="Sélectionner le grade" showSearch>
                            ...
                            <Col span={10}>
                                <Form.Item label="Chapitre">
                                    <Select
                                        ...
                                        onChange={(value) => setFormData((prev) => ({ ...prev, chapitre: value }))}
                        ...
                        <Row gutter={16}>
                            <Col span={12}>
                                <Form.Item label="Acte">
                                    <Input name="acte" value={formData.acte || ""} onChange={handleChangeMain} placeholder="Entrer l'acte" />
                                </Form.Item>
                            </Col>

                            <Col span={12}>
                                <Form.Item label="Référence">
                                    <Input name="referenceActe" value={formData.referenceActe || ""} onChange={handleChangeMain} placeholder="Entrer le reference" />
                                </Form.Item>
                            </Col>
                        </Row>

                        <Form.Item label="Date Acte">
                            <Input type="date" name="dateActe" value={formData.dateActe || ""} onChange={handleChangeMain} />
                        </Form.Item>

                        <Row gutter={16}>
                            <Col span={12}>
                                <Form.Item label="Date de Cessation de Service">
                                    <Input type="date" name="dateCessation" value={formData.dateCessation || ""} onChange={handleChangeMain} />
                                </Form.Item>
                            </Col>

                            <Col span={12}>
                                <Form.Item label="Date Fin de Paiement">
                                    <Input type="date" name="dateFinPai" value={formData.dateFinPai || ""} onChange={handleChangeMain} />
                                </Form.Item>
                            </Col>
                        </Row>
                ...
                <Col span={6} className="form-container">
                    ...
                    <Form layout="vertical">
                        <Form.Item label="Montants">
                            <Input name="montant" value={formData.montant || ""} onChange={handleChangeMain} placeholder="0000" />
                        </Form.Item>

                        <Form.Item label="Reference">
                            <Input type="text" name="referenceRecette" value={formData.referenceRecette || ""} onChange={handleChangeMain} placeholder="Entrer le reference" />
                        </Form.Item>

                        <Form.Item label="Date d'ordre de Recette">
                            <Input type="date" name="dateOrdreRecette" value={formData.dateOrdreRecette || ""} onChange={handleChangeMain} />
                        </Form.Item>

                        <Form.Item label="Date debut paiement">
                            <Input type="date" name="dateDebut" value={formData.dateDebut || ""} onChange={handleChangeMain} />
                        </Form.Item>

                        <Form.Item label="Date dernier paiement">
                            <Input type="date" name="dateDernierPai" value={formData.dateDernierPai || ""} onChange={handleChangeMain} />
                        </Form.Item>
                    ...
                    {formFields.map((field, index) => (
                        <Row gutter={8} key={index}>
                            <Col span={8}>
                                <Form.Item label="Rubrique">
                                    <Select
                                        ...
                                        onChange={(value) => handleChangeField(index, "rubrique", value)}
                            ...
                            <Col span={4} style={{ display: "flex", alignItems: "center" }}>
                                <Button type="primary" danger onClick={() => handleRemoveField(index)} className="btn">
                                    <IoMdClose />
                                </Button>
                            </Col>
                        </Row>
