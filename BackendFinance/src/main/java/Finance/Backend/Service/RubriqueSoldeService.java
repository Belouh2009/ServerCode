...
@Service
public class RubriqueSoldeService {

    @Autowired
    private RubriqueSoldeRepository rubriqueRepository;

    public void saveRubriques(List<RubriqueSolde> rubriques) {
        for (RubriqueSolde rubrique : rubriques) {
    ...
    private String generateUniqueId() {
        return "" + System.currentTimeMillis();  // Exemple de génération d'ID basé sur l'heure actuelle
    }
    ...
    public RubriqueSolde updateRubrique(String id, RubriqueSolde rubrique) throws Exception {
        Optional<RubriqueSolde> existingRubrique = rubriqueRepository.findById(id);
        
        if (!existingRubrique.isPresent()) {
            throw new Exception("Rubrique avec l'ID " + id + " non trouvée");
        }

        RubriqueSolde rubriqueToUpdate = existingRubrique.get();
