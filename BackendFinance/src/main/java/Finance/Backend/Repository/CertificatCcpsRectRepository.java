...
public interface CertificatCcpsRectRepository extends JpaRepository<CertificatCcpsRect, String>{

    // Trouver le dernier certificat basé sur l'ID
    Optional<CertificatCcpsRect> findTopByOrderByIdCertificatDesc();
    
    // Correction de la requête JPQL avec le nom exact de l'entité "CertificatCap"
    @Query("SELECT COUNT(c) FROM CertificatCcpsRect c")
